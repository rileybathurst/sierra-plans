...
const Pdfbuild = ({ name, slug, address, zip, areas, svg, notes, jobber, jobberTakedown, timerHours, timerFallback, createdAt, updatedAt, takedownFlexOrder, takedownday }: PlanTypes) => {
  ...
  doc.text(`${address}, ${areas[0]?.name}, ${areas[0]?.state === 'california' ? 'CA' : 'NV'}. ${zip ? `.${zip}` : ''}`, 0.5, 1);
  ...
  doc.save(`${jobber}${jobberTakedown ? `-${jobberTakedown}` : ''}-${name}-${slug}-${mostRecent}`); // * turn off for developing
}
